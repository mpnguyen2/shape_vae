...
from MCEVAE import calc_mcvae_loss, MCVAE 
...
X = np.load('pendulum.npy')# (N, 50, 784)
N = 500
T = 50
...
class ODE2VAE(nn.Module):
    # q: num latent dims
    def __init__(self, n_filt=8, q=8, vae_type="normal"):
        ...
        # Initialize a Multivariate Gaussian distribution with 0 mean and 1 along the diagonal of the covariance
    ...
    def forward(self, X, Ndata, L=1, inst_enc=False, method='dopri5', dt=0.1):
        ...
        for l in range(L):
            # Draw a differential function
            f = self.bnn.draw_f() 
            # Make the ODE forward function
            oderhs = lambda t,vs: self.ode2vae_rhs(t,vs,f)
            # Integrate to find the time points specified by t
    ...
    # Calculates the mean reconstruction error of the ODE2VAE for sequence of inputs
...
# plotting
def plot_rot_mnist(X, Xrec, show=False, fname='pendulum.png'):
    N = min(X.shape[0],10)
