...
def training_per_eps_without_vae(ac_opt, ac_model, T, gamma):
    # Initialize p and z
    p = np.random.randint(0, 3, size=4)
...
def train(num_iter_pretrain_vae, num_eps_with_VAE, \
          num_eps_without_VAE, initialize_interval, T,\
          discounted_rate, log_interval, learning_rate = 3e-2):
    """
    ...
    for i_episode in range(num_eps_with_VAE):
        ...
        if i_episode % log_interval == 0:
            ...
            print('Episode {}: VAE loss: {}; init_reward:{}, cumulative reward: {}; reward loss: {}; Time elapsed: {} seconds\n'\
                  .format(i_episode+1, vae_loss, init_reward, reward, torch.exp(reward_loss), time.time()-cur_time))
